...
const FacebookPermissionsSelector: React.FC<FacebookPermissionsSelectorProps> = ({
  ...
  const permissionOptions: FacebookPermissionOption[] = [
    // Basic info is still required but won't be shown in the UI
    {
      id: 'basic_info',
    ...
    },
    {
      ...
      required: true // Make ad permissions required
    },
    {
      id: 'pages',
  ...
  const [selectedPermissions, setSelectedPermissions] = useState<Record<string, boolean>>({
    ...
    ad_accounts: true, // Ad accounts is selected by default and required
    pages: false,
    instagram: false,
    insights: false,
  });
  ...
  const handleSelectAll = (checked: boolean) => {
    ...
    if (checked) {
      ...
      // Only keep required permissions
      const requiredOnly: Record<string, boolean> = {};
      permissionOptions.forEach(option => {
        requiredOnly[option.id] = !!option.required;
      });
      setSelectedPermissions(requiredOnly);
    }
  };
  ...
  const handleCheckboxChange = (permissionId: string, checked: boolean) => {
    ...
    const option = permissionOptions.find(opt => opt.id === permissionId);
    if (option && !option.required) {
      const allNonRequiredSelected = permissionOptions
        .filter(opt => !opt.required && !opt.hidden)
        .every(opt => opt.id === permissionId ? checked : selectedPermissions[opt.id]);
      
      setSelectAll(allNonRequiredSelected);
    }
  };
  ...
  return (
    <div className="space-y-4 py-4">
      ...
      <p className="text-sm text-gray-500 mb-4">
        Choose what information you want to allow this app to access. Ad account permissions are required for campaign management.
      </p>
      
      <div className="flex items-center space-x-2 p-3 border rounded-md bg-blue-50 border-blue-200 mb-2">
        <Checkbox 
          id="select-all" 
          checked={selectAll} 
